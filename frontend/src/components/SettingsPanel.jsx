...
function SettingsPanel({ models, settings, onSettingsChange }) {
  ...
  const showCustomModelInput = !models.length || !models.includes(selectedModel) || selectedModel === 'custom';
    ...
    <aside className="settings-panel">
      ...
      <div className="settings-group">
        ...
        <select
          ...
          value={selectedModel === 'custom' ? 'custom' : selectedModel}
          onChange={(e) => {
            const nextValue = e.target.value;
            if (nextValue === 'custom') {
              onSettingsChange({ ...settings, model: 'custom' });
              return;
            }
            onSettingsChange({ ...settings, model: nextValue });
          }}
        >
          {models.length === 0 ? (
